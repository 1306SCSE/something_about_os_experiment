...
实验6关于管道的设计和shell，其实是学期大总结。你可以第一次感受到操作系统的运行。

## 管道部分 ##

指导书直接的代码要求有两个文件，`user/pipe.c`和`user/sh.c`。先看管道部分：本实验划分一个内存区域在进程间共享，分配文件控制块映射到这个内存区域，因此这个区域就是管道。我们把管道视作文件，为了标记两个进程不同的读写状态，这个内存区域用文件控制块来管理。

为了设计管道，首先需要了解管道如何使用。管道是内存中的文件，内容如下
```c
struct Pipe {
...
```
这个管道必须要在两个进程间共享。引入共享内存权限标志`PTE_LIBRIRAY`。一个管道文件对应读文件控制块编号`p[0]`和写文件编号`p[1]`。

测试管道是否创建正确的程序是`user/testpipe.c`
```c
void
umain(void)
{
...
```
由上面的程序我们知道`fork()`在处理`PTE_LIBRARY`权限时不能设定为COW而是直接共享映射。
...
接下来完成管道读写操作。用户调用`read(int fdnum, void *buf, u_int n)`从`fdnum`编号的文件里面读文件。要从一个只知道文件控制块编号的东西里面读文件，首先得知道这个文件控制块地址吧？所以调用`fd_lookup(fdnum, &fd)`找到文件控制块地址，找到文件控制块地址，还得知道文件在什么设备上，文件控制块里面的`fd->dev_id`记录设备`dev_id`，有了设备`dev_id`还得找到设备控制块的地址，调用`dev_lookup(fd->fd_dev_id, &dev)`找到设备控制块。不同设备需要有不同的读写函数，因此设备控制块保存了不同设备的读写函数。比如管道的设备控制块
```c
struct Dev devpipe =
{
.dev_id=	'p',
...
```
每次只传输一个字节，当管道空`p_rpos>=p_wpos`时检查管道是否已经关闭，关闭时返回或者读取到足够多字节时返回。每一次没有读到任何数据，可以考虑`syscall_yield()`。`testpipe`里面只能是管道另一端关闭时关闭。
...
接下来在`init/init.c`中先后填入`ENV_CREATE(user_testpipe)`和`ENV_CREATE(user_testpiperace)`完成测试，`icode`的起始地址仍然是`UTEXT+0x1000`，测试通过的标志可以在对应的文件中找到。两个测试都看到`successed`之后就可以进入下一步了。
...
启动此系统需要在`init/init.c`中创建`user_icode`和`fs_serv`进程，需要注意的是文件系统读写`user/fsipc.c`中所有的信号均发送给`envs[1]`，所以请确保`fs_serv`对应`envs[1]`。不出意外，测试时就会出现
```
